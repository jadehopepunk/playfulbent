  ...
  module ViewHelpers
    ...
    def will_paginate entries = @entries, options = {}
      ...
      if total_pages > 1
        ...
        links.unshift(page_link_or_span(entries.previous_page, 'disabled', options.delete(:prev_label))) unless entries.current_page <= 1
        links.push(page_link_or_span(entries.next_page, 'disabled', options.delete(:next_label))) unless entries.current_page >= entries.page_count
