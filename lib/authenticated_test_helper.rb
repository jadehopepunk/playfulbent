module AuthenticatedTestHelper
  ...
  def login_as_mock(user)
    @request.session[:user] = '666'
    #Application.stubs(:find_user_from_session).returns(user)
    User.stubs(:find_by_id).with('666').returns(user)
  end
  ...
  def assert_requires_access(login = nil)
    yield HttpForbiddenProxy.new(self, login)
  end
...
class HttpLoginProxy < BaseLoginProxy
  protected
    ...
    def check
      @controller.assert_redirected_to :controller => 'session', :action => 'new'
    end
end

class HttpForbiddenProxy < BaseLoginProxy
  protected
    def authenticate
      @controller.login_as @login if @login
    end
    ...
    def check
      @controller.assert_response 403
    end
end
