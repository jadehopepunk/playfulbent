...
class Flickr::Auth < Flickr::APIBase
	...
	def login_link(perms = 'delete', existing_frob = nil)
		args={ 'api_key' => @flickr.api_key, 'perms' => perms}
		args['frob'] = existing_frob if existing_frob
	...
	def getFullToken(mini_token)
		res = flickr.call_unauth_method('flickr.auth.getFullToken', 'mini_token' => mini_token)
		@token = Flickr::Token.from_xml(res)
	end
	...
	def checkToken(token=nil)
		...
		raise token.inspect
		res = @flickr.call_unauth_method('flickr.auth.checkToken', 'auth_token' => token)
		@token = Flickr::Token.from_xml(res)
	end
end
